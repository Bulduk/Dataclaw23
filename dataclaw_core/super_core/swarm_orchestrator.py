...
class AgentNode:
    ...
    def delegation(self, required_capability: str, market_data: Dict[str, Any], confidence_threshold: float = 0.5) -> Dict[str, Any]:
        """Delegate a task to another specialized agent in the swarm based on capability and confidence constraints."""
        ...
        return self.swarm_reference.request_delegation(self.name, required_capability, market_data, confidence_threshold)

    def delegate_task(self, required_capability: str, market_data: Dict[str, Any], confidence_threshold: float = 0.5) -> Dict[str, Any]:
        """Alias for backward compatibility"""
        return self.delegation(required_capability, market_data, confidence_threshold)
...
class SwarmOrchestrator:
    ...
    def request_delegation(self, from_agent: str, required_capability: str, market_data: Dict[str, Any], confidence_threshold: float = 0.5) -> Dict[str, Any]:
        ...
        capable_agents = [a for a in self.agents if required_capability in a.capabilities and a.name != from_agent and a.weight >= confidence_threshold]
        if not capable_agents:
            logger.warning(f"No agents found or none passed confidence threshold ({confidence_threshold}) for delegated task: {required_capability}")
            return {"signal": "HOLD", "confidence": 0.0, "rationale": "Delegation failed - insufficient confidence or capability"}
        ...
        logger.info(f"Task delegated from {from_agent} to {best_agent.name} with confidence {best_agent.weight}")
